...
**Olympic rings bias in HF circles.** The HF dataset uses an Olympic-style interlocking layout (rows of overlapping circles). The model has a strong prior that this layout = 5 circles, regardless of color. When GT≠5, the model predicts "5" in **22.4%** of HF cases (both monochrome and colored) vs **0%** in generated images (which use randomized non-interlocking layouts).

| GT | HF "predicted 5" (monochrome) | HF "predicted 5" (colored) | Gen "predicted 5" rate |
|----|-------------------------------|---------------------------|----------------------|
| 6 | 27.1% | 37.5% | 0% |
| 7 | 20.8% | 41.7% | 0% |
| 8 | 18.8% | 14.6% | 0% |
...
The bias is **layout-driven, not color-driven**: monochrome HF images trigger predict-5 at 20-27% for GT 6-7. Color amplifies the effect by ~10-20pp but is not required — the model explicitly names monochrome images as "Olympic rings" in its responses.
...
**Systematic +2 overcount at overlap=0.3.** The count×overlap cross-tab reveals a striking error pattern at ov=0.3: the model overcounts by exactly +2 for counts 4–8 (count=5 → all 10 errors say 7; count=6 → all 10 say 8). The overlap geometry at 0.3 consistently creates ~2 "phantom" pentagons from intersection regions that the model interprets as additional shapes. Count=9 at ov=0.3 is 100% correct — not because the model counts better, but because two opposing error pressures cancel: the +2 overcount effect collides with density-driven undercounting as shapes begin to merge. This is confirmed by count=10 at ov=0.3, which flips to systematic undercounting (all 9 errors say 9, i.e., -1). The transition from overcounting to undercounting happens precisely at the count=9 boundary, making 100% accuracy there a coincidence of competing biases rather than genuine counting ability.
...
| HF Blind (blank + word) | 64.8% | 98.9% | 100% | 528 |
...
**2. Olympic rings memorization bias (circles only).** The HF circle task uses an interlocking 2-row layout that the model recognizes as "Olympic rings" regardless of color. The model defaults to "5" in 22.4% of non-5 HF cases (both monochrome and colored) vs 0% in generated images, which use randomized non-interlocking layouts. Color amplifies the bias (~10-20pp higher predict-5 rates) but the layout alone is sufficient. This accounts for **73% of the circle gap** (17.9 of 24.4pp). Removing bias-driven errors brings HF to 83.5%, within 6.5pp of gen's 90%. This is a memorization failure, not a perceptual one — the same phenomenon studied in the VLMs Are Biased paper.
...
4. **Memorization priors override visual evidence.** The HF circle task's interlocking 2-row layout triggers a strong "5 rings" prior, accounting for 73% of the gen-vs-HF accuracy gap. The model predicts "5" for 32% of 6-circle and 31% of 7-circle HF images — in both monochrome (27%/21%) and colored (38%/42%) variants. The bias never appears in generated images, which use randomized non-interlocking layouts. This is a layout-driven memorization failure, not a color or perceptual one, and connects directly to the VLMs Are Biased findings.
